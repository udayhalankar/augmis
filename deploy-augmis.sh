...
: "${DATABASE_URL:?Set DATABASE_URL before running deploy-augmis.sh}"
...
EOFENV
if [ -n "${AUGMIS_CONNECTOR_SECRET_KEY:-}" ]; then
  echo "AUGMIS_CONNECTOR_SECRET_KEY=${AUGMIS_CONNECTOR_SECRET_KEY}" >> /home/ec2-user/augmis/backend.env
fi
